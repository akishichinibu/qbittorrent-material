...
interface HeaderType {
  title: string;
  dataIndex: keyof QBTorrentInfo;
  render: (value: any, data: QBTorrentInfo, index: number) => JSX.Element;
}


// const columns: Array<[string, keyof QBTorrentInfo, (value: any) => JSX.Element]> = [
//   ["Name", "name", (r) => <span>{r}</span>],
//   ["Size", "size", (r) => <span>{fileSizePretties(r)}</span>],
//   ["Ratio", "ratio", (r) => <span>{`${Math.floor(info.downloaded / info.size * 10000) / 100}%`}</span>],
//   ["Name", "name", (r: string) => <span>{text}</span>],
//   ["Name", "name", (r: string) => <span>{text}</span>],
//   ["Name", "name", (r: string) => <span>{text}</span>],
// ]
...
const columns: HeaderType[] = [
  ...
  },
  {
    title: "Size",
    dataIndex: "size",
    render: (r) => <span>{fileSizePretties(r)}</span>,
  },
  {
    title: "Ratio",
    dataIndex: "ratio",
    render: (_, o) => <span>{`${Math.floor(o.downloaded / o.size * 10000) / 100}%`}</span>,
  },
  {
    title: "下载速度",
    dataIndex: "dlspeed",
    render: (r) => <span>{`${fileSizePretties(r)}/s`}</span>,
  },
  {
    title: "上传速度",
    dataIndex: "upspeed",
...
const TorrentsTable2: FC = () => {
  ...
  const data = Object.entries(torrents).map(([hash, r]) => ({
    key: hash,
    ...r.info,
  }));
    ...
    <div>
      ...
      <Table
        ...
        rowSelection={{
          fixed: true,
          type: "radio",
          onSelect: (keys) => handleClick(keys.key),
        }}
      />
    </div>
  </>
